...
public class ChatStompInterceptor implements ChannelInterceptor {
    ...
    private void authorizeSend(StompHeaderAccessor accessor) {
        ...
        if (accessor.getDestination() == null
                || !SEND_DESTINATION.matcher(accessor.getDestination()).matches()) {
    ...
    private Message<?> authenticate(Message<?> message, StompHeaderAccessor accessor) {
        ...
        try {
            authMember = jwtTokenProvider.parseAccessToken(
                    authorization.substring(BEARER_PREFIX.length()));
        } catch (InvalidJwtException exception) {
            throw new StompAuthenticationException(
                    StompAuthenticationException.Reason.INVALID_TOKEN);
        }
        if (authMember.role() != MemberRole.MEMBER) {
            throw new StompAuthenticationException(
    ...
    private void authorizeSubscribe(StompHeaderAccessor accessor) {
        ...
        ReservationChatAccessPort.ChatAccess access = reservationChatAccessPort.read(
