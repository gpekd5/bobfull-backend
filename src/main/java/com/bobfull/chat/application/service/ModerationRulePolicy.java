...
public class ModerationRulePolicy {
    private static final Pattern MOBILE_PHONE =
            Pattern.compile("(?<!\\d)010[\\s.-]*\\d{4}[\\s.-]*\\d{4}(?!\\d)");
    private static final Pattern PERSONAL_PHONE_CONTEXT =
            Pattern.compile("(내 번호|제 번호|내 연락처|제 연락처)");
    private static final Pattern EXACT_PROFANITY =
            Pattern.compile("^(씨발|시발|시[\\s.\\-@]+발|개새끼(야)?|죽여버린다)$");
    private static final Pattern COIN_INDUCEMENT =
            Pattern.compile("코인 수익방.*(들어오세요|가입하세요|참여하세요|신청하세요)");
    private static final Pattern STOCK_INDUCEMENT =
            Pattern.compile("주식 리딩방.*(들어오세요|가입하세요|참여하세요|신청하세요|종목을 알려드립니다)");
    private static final Pattern LOAN_INDUCEMENT =
            Pattern.compile("대출 승인 보장.*(신청하세요|가입하세요)");

    public Optional<ModerationResult> clearFlagged(String content) {
        if (isPromptInjectionCandidate(content)) {
            return Optional.empty();
        }
        boolean personal = MOBILE_PHONE.matcher(content).find()
                && PERSONAL_PHONE_CONTEXT.matcher(content).find()
        ...
        if ((personal ? 1 : 0) + (profanitySignal ? 1 : 0) + (spamSignal ? 1 : 0) > 1) {
            return Optional.empty();
        }
    ...
    Optional<ModerationResult> clearSplitFlagged(List<String> canonicalCandidates) {
        return canonicalCandidates.stream()
                .map(this::clearSplitFlagged)
                .flatMap(Optional::stream)
                .findFirst();
    }

    private static Optional<ModerationResult> flagged(ModerationCategory category, RiskLevel riskLevel) {
        return Optional.of(new ModerationResult(
                ModerationResultType.FLAGGED, Set.of(category), riskLevel));
    }

    private static boolean isPromptInjectionCandidate(String content) {
        return content.contains("이전 지시")
                || content.contains("이전 명령")
                || content.contains("System Prompt")
                || content.contains("JSON Schema")
                || content.contains("Moderation AI")
                || content.contains("{\"result\":\"SAFE\"}")
                || content.contains("관리자 권한")
                || content.contains("분석하지 말고")
                || content.contains("정책을 공개")
    ...
    private static boolean hasProfanitySignal(String content) {
        return content.contains("씨발")
                || content.contains("시발")
                || content.contains("개새끼")
                || content.contains("죽여버린다");
    }
