...
public record RestaurantFeedbackInsightResponse(
        String category,
        String aspectType,
    ...
    public static RestaurantFeedbackInsightResponse from(Aggregation result) {
        ...
        String summary = keepsLlmAspect
                ? result.getAspect() + " " + opinionKorean(result.getOpinionType().name())
                        + "에 대한 " + sentimentText + " 의견 " + result.getSenderCount() + "명"
                : result.getAspect() + "에 대한 " + sentimentText + " 의견 " + result.getSenderCount() + "명";
        return new RestaurantFeedbackInsightResponse(
                result.getCategory().name(),
                result.getAspectType().name(),
                result.getAspect(),
                result.getOpinionType().name(),
                result.getSentiment().name(),
                result.getSenderCount(),
                summary
        );
    }

    private static String sentimentKorean(String sentiment) {
    ...
    private static String opinionKorean(String opinion) {
        return switch (opinion) {
            ...
            case "FRESHNESS" -> "신선도";
            case "TEMPERATURE" -> "온도";
            case "FRIENDLINESS" -> "친절";
            case "SERVICE_SPEED" -> "응대 속도";
            case "PRICE_LEVEL" -> "가격";
            case "CLEANLINESS" -> "청결";
            case "WAITING" -> "대기 시간";
            default -> "평가";
