...
public class SpringAiRestaurantFeedbackInsightAdapter implements RestaurantFeedbackInsightPort {
    ...
    public SpringAiRestaurantFeedbackInsightAdapter(
            @Qualifier("restaurantInsightChatClient") ChatClient restaurantInsightChatClient
    ) {
        this.chatClient = restaurantInsightChatClient;
    }

    @Override
    public RestaurantFeedbackAnalysisResult analyze(String content) {
        ResponseEntity<ChatResponse, RestaurantFeedbackAnalysis> response = chatClient.prompt()
                .system(RestaurantFeedbackPrompt.SYSTEM_PROMPT)
                .user(content)
                .call()
                .responseEntity(
                        RestaurantFeedbackAnalysis.class,
        ...
        return new RestaurantFeedbackAnalysisResult(response.entity(), "OpenAI", model);
    }
}
