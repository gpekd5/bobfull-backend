...
public class RestaurantInsightConsumerConfig {
    ...
    @Bean
    CommonErrorHandler restaurantInsightErrorHandler(
            RestaurantInsightDltRecoverer recoverer,
            @Value("${bobfull.kafka.restaurant-insight.consumer-max-attempts:3}") int attempts,
            @Value("${bobfull.kafka.restaurant-insight.consumer-retry-backoff-ms:1000}") long backoff
    ) {
        DefaultErrorHandler handler = new DefaultErrorHandler(
                recoverer,
                new FixedBackOff(backoff, Math.max(0, attempts - 1))
        );
        handler.addNotRetryableExceptions(CustomException.class, InvalidChatMessageEventException.class);
        return handler;
    }
