...
import com.bobfull.payment.application.port.PortOneRefundPort.ReconciliationStatus;
import com.bobfull.payment.application.port.PortOneRefundPort;
import com.bobfull.payment.infrastructure.config.PortOneProperties;
...
class PortOneRefundGatewayAdapterTest {
    ...
    void REST_환불요청의_FAILED만_명시적실패다() {
        ...
        assertThatThrownBy(() -> restAdapter(builder.baseUrl("https://api.portone.io").build()).request(PAYMENT_ID, REFUND_AMOUNT, "reason", "raw-key-123456789")).isInstanceOf(PortOneRefundPort.ExplicitRefundFailureException.class); server.verify();
    }
    ...
    void SUCCEEDED_응답에_cancellationId가_없으면_명시적실패가_아니다() {
        ...
        assertThatThrownBy(() -> restAdapter(builder.baseUrl("https://api.portone.io").build()).request(PAYMENT_ID, REFUND_AMOUNT, "reason", "raw-key-123456789")).isNotInstanceOf(PortOneRefundPort.ExplicitRefundFailureException.class);
        server.verify();
    }
