...
`200 OK
{
  ...
  "data": {
    ...
    explanation: {
      ...
      whySequence: ["System Flow의 마지막 단계 — 클라이언트가 최종적으로 받는 응답."],
      ifFails: ["해당 없음."],
      fact: []
    }
  });
  ...
  const steps = [
    {
      ...
      stateChanges: {},
      explanation: {
        whatRuns: [`${roleKorean}이 이메일·비밀번호로 로그인 요청을 보낸다(System Flow 내부 가상 테스트 데이터).`],
        whatChanged: [], safeguards: [], whySequence: [], ifFails: [],
...
const STATUS_BADGES = {
  ...
  pending: { label: "System Flow 상세 연결 준비 중", cls: "status-pending" },
...
function renderRolePanel() {
  ...
  body.appendChild(el("p", { class: "sim-note", text: "위 입력값은 System Flow 내부 가상 테스트 데이터입니다. 실제 서버 요청은 전송되지 않습니다." }));
