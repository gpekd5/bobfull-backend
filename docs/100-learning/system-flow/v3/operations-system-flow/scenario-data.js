...
const fullArchitectureNodeDetails = {
  ...
  redis: { role: "Cache + Pub/Sub(다중 서버 채팅 전파)", runtime: "ElastiCache for Valkey, TLS(REDIS_SSL_ENABLED) — Redis 호환 프로토콜", connectedTo: "Application(Blue/Green) 전체", network: "Private, Private Subnet 2a/2c", evidence: "docs/040-architecture/adr/0014-shared-redis-elasticache.md, docs/110-records/evidence/v3/169-app-ha/README.md" },
  ...
  lambda: { role: "업로드 이미지 검증", runtime: "S3 ObjectCreated 트리거(temp/restaurants/** → 검증 후 최종 경로로 복사)", connectedTo: "S3", network: "—", evidence: "lambda/restaurant-image-validator, docs/040-architecture/adr/0007" },
  ...
  portone: { role: "결제 · 환불 게이트웨이", runtime: "Webhook: /api/webhooks/portone", connectedTo: "Application", network: "External HTTPS", evidence: "PortOneConfig, docs/040-architecture/architecture.md" },
